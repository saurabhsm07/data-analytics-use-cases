...
def get_vals_based_on_corr_groups(titanic_train, top_corr_dict, attr_type):
    ...
    for attr in top_corr_dict.keys():
        if attr_type[attr] == 'category':
            df = titanic_train.groupby(top_corr_dict[attr].index.tolist(), as_index= False)[attr].agg(lambda x : x.value_counts().index[0] if len(x.value_counts()) is not 0 else titanic_train[attr].value_counts().index[1])
        else:
            df = titanic_train.groupby(top_corr_dict[attr].index.tolist(),  as_index= False)[attr].agg(lambda x : x.mean() if pd.isnull(x.mean()) is not True else titanic_train[attr].value_counts().index[0])
